...
void main_demo(void)
{
	...
	xTaskCreate(SerialSend_Task0, "STx", configMINIMAL_STACK_SIZE, NULL, TASK_SERIAL_SEND_PRI, NULL);
	xTaskCreate(SerialSend_Task1, "STx", configMINIMAL_STACK_SIZE, NULL, TASK_SERIAL_SEND_PRI, NULL);
...
void SerialSend_Task0(void* pvParameters)
{
	...
	for (;;)
	{
		...
		if (send_serial_character(COM_CH, c) != 0)
		{
...
void SerialSend_Task1(void* pvParameters)
{
	...
	{
		vTaskDelay(pdMS_TO_TICKS(200));
		if (send_serial_character(COM_CH1, c) != 0)
		{
			printf("Greska prilikom slanja");
		}
	}
